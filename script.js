...
const FORM_ENDPOINT = "https://script.google.com/macros/s/AKfycbx0VxZnVh0YAqdJd4cPRlJWHKnIsUZdWbiWCERRoJXHI0JwEeZoqDeWRHFrRgZEkkTR/exec";
...
(function initRsvpForm() {
  ...
  async function submitToEndpoint(payload) {
    // У Google Apps Script Web App нет поддержки CORS-preflight для
    // Content-Type: application/json — браузер блокирует такой запрос ещё
    // до отправки. text/plain — «простой» запрос, preflight не требуется,
    // а Apps Script (telegram-relay.gs) читает e.postData.contents как есть
    // и парсит JSON независимо от заголовка. Formspree/Make ждут именно
    // application/json, поэтому переключаем заголовок только для Apps Script.
    ...
    // Apps Script (ContentService) всегда отвечает HTTP 200, даже если внутри
    // telegram-relay.gs произошла ошибка (например, не заданы свойства
    // скрипта) — поэтому для него дополнительно проверяем { ok: true/false }
    // в теле ответа, иначе реальная ошибка отправки в Telegram будет молча
    // показана гостю как «Спасибо! Анкета отправлена».
    if (isAppsScript) {
      const result = await response.clone().json().catch(() => null);
      if (!result || result.ok !== true) {
        throw new Error('Apps Script relay error: ' + (result && result.error ? result.error : 'unknown'));
      }
    }
    return response;
  }
